...
import com.rodionov.child_education.app.ChildEducationApp
...
val Uri.realName: String get() {
    try {
        ...
        ChildEducationApp.context.contentResolver
...
val Uri.realPath: String? get() {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT && DocumentsContract.isDocumentUri(ChildEducationApp.context, this)) {
...
fun Uri.getDataColumn(selection: String? = null, selectionArgs: Array<String>? = null): String? {
    ...
    try {
        ChildEducationApp.context.contentResolver.query(this, projection, selection, selectionArgs, null)?.use { cursor ->
            if (cursor.moveToFirst()) {
...
val Uri.sha256: String get() {
    ChildEducationApp.context.contentResolver.openInputStream(this)?.use {
